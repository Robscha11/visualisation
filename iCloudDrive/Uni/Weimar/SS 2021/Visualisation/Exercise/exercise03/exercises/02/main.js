...
loadMoviesDataset().then((movies) => {
  ...
  const topMovies = movies
    .filter((d) => d.imdbVotes > 150000)
    .sort((a, b) => d3.descending(a.imdbRating, b.imdbRating))
    .slice(0, 10);
  ...
  //console.log(movies.filter(d => d.key < 10))
  //console.log(d3.rollups(movies, v => v.length, d => d.originalLanguage));
  const movieCountByLanguage = d3
    .rollups(
      movies,
      (v) => v.length,
      (d) => d.originalLanguage
    )
    .map((d) => {
      d;
      const define = {};
