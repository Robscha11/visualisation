...
export function smallMultiples({ svg, width, movies, numCols } = {}) {
  ...
  //
  // moviesPerYear: a map from each unique year to the mean and count of all
  //                movies for that year
  //
  // moviesByGenrePerYear:  a map from each unique genre with another nested map
  //                        inside that is similar to moviesPerYear but limited
  //                        to movies with that respective genre
  let genres = new Set(["Action", "Drama", "Western"]);
  let moviesPerYear = new Map([
    [1930, { count: 10, mean: 5 }],
    [1980, { count: 5, mean: 9 }],
    [2020, { count: 100, mean: 7 }],
  ]);
